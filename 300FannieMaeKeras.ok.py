...
# too long to run
pplotdf1 =myspark.sql('select seller_name, float(channel),float( intrate) ,float(loanamt),float(loan2val), float(numborrowers), float(creditscore),  float(fcl_costs) from loandata where fcl_costs<.8 limit 5000')
pplotdf1.show(3)
...
output.show(1)
kerasinputdf=output.drop('property_state','features','origination_date', 'loan_identifier','seller_name')
kerasinputdf.show(1)
kerasinputdf.count()
...
# moving to 32
classifier.fit(X,y, batch_size=32, epochs=50, verbose=1)
